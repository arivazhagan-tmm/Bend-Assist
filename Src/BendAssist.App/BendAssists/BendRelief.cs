...
public sealed class BendRelief : BendAssist {
   ...
   public override void Execute () {
      ...
      foreach (var vertex in mCommonVertices) {
         ...
         PLine nearBaseEdge = GetNearBaseEdge (connectedLines, vertex, center, IsHorizontalRange, IsVerticalRange); // Get the near base edge where relief edges will be inserted
         angle = angle switch { 180 => 0, 270 => 90, _ => bl.Angle, };
         (double translateAngle1, double translateAngle2) = GetTranslateAngles (vertex, center, angle, IsHorizontalRange, IsVerticalRange);
         p2 = vertex.RadialMoved (brHeight, translateAngle1);
         p3 = p2.RadialMoved (brWidth, translateAngle2);
         p4 = FindIntersectPoint (nearBaseEdge, p3, translateAngle1 - 180);
   ...
   Point2 FindIntersectPoint (PLine pLine, Point2 p, double angle) {
      Point2 p1 = p.RadialMoved (p.DistToLine (pLine.StartPoint, pLine.EndPoint), angle);
      return p1;
   ...
   PLine GetNearBaseEdge (List<PLine> connectedLines, Point2 vertex, Point2 center, bool IsHorizontalRange, bool IsVerticalRange) {
      if (IsHorizontalRange)
         return (vertex.X < center.X && vertex.Y > center.Y) || (vertex.X > center.X && vertex.Y < center.Y)
            ? connectedLines[1] : connectedLines.First ();
      if (IsVerticalRange)
         return (vertex.Y < center.Y && vertex.X < center.X) || (vertex.X > center.X && vertex.Y > center.Y)
            ? connectedLines[1] : connectedLines.First ();
      return null!;
   }

   Tuple<double, double> GetTranslateAngles (Point2 vertex, Point2 center, double angle, bool IsHorizontalRange, bool IsVerticalRange) {
