...
public sealed class CornerClose : BendAssist {
   ...
   public override void Execute () {
      var (vertices, pLines, bendLines) = (mPart!.Vertices, mPart.PLines, mPart.BendLines.Cast<Line> ().ToList ());
      ...
      var bls = new List<Line> ();
      foreach (var vertex in commonVertices) {
         vertex.IsCommonVertex (bendLines, out bls);
         if (bls.All (b => b.Orientation != Inclined)) mCcVertices.Add (vertex);
      }
      if (mCcVertices.Count < 1) return;
      ...
      foreach (var pLine in pLines)
         if (mCcVertices.Any (pLine.HasVertex)) mStepLines.Add (pLine);
