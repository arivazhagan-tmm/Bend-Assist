...
namespace BendAssist.App.FileHnadling;
...
public class GeoReader (string fileName) {
    ...
    public Part ParsePart () {
        ...
        while (ReadLine (out line)) {
            switch (line) {
                case "#~11": // Gets material details
                    ...
                    materialType = line;
                    ReadLine (out line);
                    thickness = ParseInfo (line);
                    break;
                case "#~31": // Gets vertices
                    while (ReadLine (out line) && line is "P") {
                        SkipLine ();
                        ReadLine (out line);
                ...
                case "#~37": // Gets bendline info (Radius,Angle,Deduction)
                    SkipLine ();
                    ReadLine (out line);
                    bAngle = ParseInfo (line.Split (' ')[0]);
                    ReadLine (out line);
                    bRadius = ParseInfo (line.Split (' ')[0]);
                    ReadLine (out line);
                    bDeduction = -1 * ParseInfo (line);
                    break;
        ...
        bool ReadLine (out string str) { // Reads the line and return the trimmed line if it is not null
            str = reader.ReadLine ()!;
        ...
        void SkipLine () => reader.ReadLine (); // Skips a line
    }
    #endregion
